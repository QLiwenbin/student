...
function $(selet,ranger=document){
    if(typeof selet == 'string'){
        let str1 = selet.trim();
        ...
        }else if(/^[a-zA-Z][A-Za-z1-6]{0,7}$/.test(str1)){
            return ranger.getElementsByTagName(str1);
        }else if(/^<[a-zA-Z][A-Za-z1-6]{0,7}>$/.test(str1)){
            return document.createElement(str1.slice(1,-1));
        }
    }else if(typeof selet == 'function'){
        window.addEventListener('load',selet);
    }
}
